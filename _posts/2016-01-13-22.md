...
it is a state with higher *entropy*, or disorder.
However, as ions of a particular charge spread out, they create a difference in charge.
This problem is particularly acute in the cell, where negatively charged molecules,
...
This charge difference establishes an electrical potential --
a field that tries to equalize charge inside and outside the cell.

The result of putting these two forces in conflict with one another is an equilibrium --
a point at which the flows caused by each force individually exactly cancel each other out.
...
For a single ion, the process is straightforward enough that I have reproduced a sketch derivation below, for the mathy types.
...
equal to the charge, in coulombs, per mole of electrons.
...
if we're talking standard neuroscience units, e.g. milliVolts:
...
*z* is the charge of the ion, which is +1, -1, or +2 in a neuroscience context.

So the primary determinant of the equilibrium potential for an ion is the ratio of
...
In the preceding discussion, we only considered cells with membranes permeable to one ion.
...
If there is a very large concentration difference, but no or very few holes in the membrane to flow through,
then very few ions will flow --
imagine a water balloon with a pinprick hole at the bottom.
We need to take the relative *permeabilities* of the ions into account.

The resulting equation assuming all ions have a charge of 1 or -1,
the *Goldman-Hodgkin-Katz Equation*, appears below:

$$
\begin{align*}
        \Delta V &= \frac{25}{z}\ln\left(\frac{\sum_{+}{p_x[X]_o}+\sum_{-}{p_x[X]_i}}{\sum_{+}{p_x[X]_i}+ \sum_{-}{p_x[X]_o}}\right)
\end{align*}
$$

Here *p* stands for permeability.
Note that we're doing two separate sums, one for positive ions and one for negative ions.
Note that the arguments are different for these two sums:
...
-- it allows variable amounts of current through --
...
![Equivalent Circuit (from Kandel)]({{site.url}}/images/equivalentCircuit.png)
...
[Capacitive Current]: {{site.url}}/images/capcurrent_1.jpg
...
[River Scenario]: {{site.url}}/images/capriver_1.jpg
{: height="318px" width= "285px"}
...
If, on the other hand, the river is very wide, the whole exercise will prove futile much more quickly.
...
\begin{align*}
	k_e c(x) \nabla \Phi(x)& = - k_d \nabla c(x) \\
	k_e \int_i^o{\Phi(x) dx} &= -k_d \int_i^o \frac{dc}{c(x)}  \\
